...
def build_transcript(
    ...
    """Build a human-readable debate transcript for logging and retry context."""
    W = 72
    lines = [
        "=" * W,
...
def _query_regulation(query: str) -> str:
    ...
    checks = [
        ...
        ("HIPAA",     ["hipaa", "protected health information", " phi", "ephi"]),
...
def _filter_evidence_pool(
    ...
    """Remove off-regulation chunks from the evidence pool before the judge sees it."""
...
def _confidence_signal(claims: List[Claim]) -> float:
    """Min-mean conservative aggregate for per-cycle confidence signal."""
