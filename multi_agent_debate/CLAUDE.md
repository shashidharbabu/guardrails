...
├── rag/                       ← RAG pipeline (Qdrant, Qwen3-4B embedder)
...
**Stack:** Qdrant (GCP) + Qwen3-4B embedder + BM25 hybrid + cross-encoder reranker.

**Corpus:** 72+ regulatory documents including GDPR, HIPAA, HITECH, CCPA,
NIST frameworks, EU AI Act, OWASP Top 10. Tiered by authority:
```
Tier 1 — Primary law / statute       (GDPR, HIPAA, ADA, CCPA)
Tier 2 — Official guidance            (EDPB, HHS OCR, FTC guidance)
Tier 3 — Framework / best practice    (NIST, OWASP, ISO)
Tier 4 — Commentary / unofficial      (industry papers)
```

**Retrieval pipeline:** Dense vector search → BM25 hybrid → cross-encoder rerank
(ms-marco-MiniLM-L-6-v2, nDCG@10: 0.891)
...
**CHUNKS_JSONL_PATH** — set this env var to point at your local JSONL export.
If not set, `multi_agent/rag_stub.py` falls back to built-in Healthcare samples.
...
## Environment variables (all optional except TAVILY removed)
...
# RAG
CHUNKS_JSONL_PATH=/path/to/rag/chunks.jsonl   # falls back to built-in samples if not set
...
Note: `TAVILY_API_KEY` was removed from the MAD pipeline. Agent B uses
asymmetric RAG only (no web search). This is an intentional design decision.
...
# 7. Swap in real Qdrant (when ready)
# Edit ONLY the body of multi_agent/rag_stub.retrieve()
# Everything else calls retrieve() unchanged
```
...
4. Tier-differentiated retrieval for TIER_OVERRIDE challenges
5. Gap-finding obligation combined with CFMAD critic pattern
