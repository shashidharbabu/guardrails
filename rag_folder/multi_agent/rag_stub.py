...
NOTE ON TIER
------------
Corpus tier metadata (T0/T1/T2/T3) is a batch/collection label, not an
authority ranking — OWASP and ISO 27001 are both T0 in this corpus.
Tier filtering is therefore NOT used. The EvidenceChunk.tier field is
kept for future use once corpus has a reliable authority_level field.

HEALTHCARE DOMAIN
-----------------
Sample chunks cover the Healthcare domain (your paper's primary eval domain)
plus GDPR/HIPAA for cross-regulation testing.
"""
...
def retrieve(
    ...
    """
    Keyword TF-IDF-lite retrieval over loaded chunks.

    SWAP POINT — to use real Qdrant, replace this entire function with:

        from rag.retriever import retrieve  # noqa: F401

    That single line makes rag_stub.retrieve point to the real implementation.
    agent_a, agent_b, judge, debate_engine call retrieve() unchanged.

    TO ACTIVATE QDRANT NOW:
        1. Set env vars: QDRANT_URL, QDRANT_API_KEY (or they default from rag/config.py)
        2. Set env var:  HF_TOKEN (if nvidia/llama-embed-nemotron-8b is gated)
        3. Replace this function body with the single import line above
        4. Run: python -m multi_agent.run_test
    """
    chunks = _load_chunks()
    if not chunks:
        return []
