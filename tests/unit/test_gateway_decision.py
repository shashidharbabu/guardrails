...
RAW = "test input"  # decide() requires raw_input as first positional arg
...
def test_hard_block_on_max_jailbreak(engine):
    ...
    result = engine.decide(RAW, pii_score=0.0, jb_score=0.95, pi_score=0.0)
    assert result.decision == Decision.BLOCK


def test_hard_block_on_max_pi(engine):
    """pi_score >= 0.7 triggers immediate BLOCK (hard override)."""
    result = engine.decide(RAW, pii_score=0.0, jb_score=0.0, pi_score=0.85)
    assert result.decision == Decision.BLOCK


def test_hard_block_on_high_pii(engine):
    """pii_score >= 0.9 triggers immediate BLOCK (hard override)."""
    result = engine.decide(RAW, pii_score=0.95, jb_score=0.0, pi_score=0.0)
    assert result.decision == Decision.BLOCK


def test_allow_clean_input(engine):
    """All low scores → PASS."""
    result = engine.decide(RAW, pii_score=0.01, jb_score=0.02, pi_score=0.01)
